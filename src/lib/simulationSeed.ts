...
export function getSimulationSeedResult(
  ...
  return {
    ...
    usesExistingApiKey: hash % 3 === 0,
    preResonancePoints: 1 + (hash % 2),
    resonanceOutcome: rewardValue >= 23 ? "strong_success" : "success",
    resonancePointsEarned: rewardValue >= 23 ? 10 : 5,
    currentPointsTotal: (1 + (hash % 2)) + (rewardValue >= 23 ? 10 : 5),
    remainingPoints: ((1 + (hash % 2)) + (rewardValue >= 23 ? 10 : 5)) - 2,
    leaderboardRank: 3 + (hash % 5),
    txId,
